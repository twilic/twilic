#!/usr/bin/env python3
"""Emit R protocol_io_messages.R from twilic-ruby protocol.rb (write/read helpers)."""
...
# Hand-maintained port of TwilicCodec I/O helpers (from twilic-ruby / twilic-lua).
CONTENT = r'''# Protocol message I/O helpers (ported from twilic-ruby protocol.rb)
  ...
  if (!is.null(column$dictionary_id) && column$values$kind == ElementTypeSTRING) {
    if (column$codec %in% c(VectorCodecDICTIONARY, VectorCodecSTRING_REF)) {
      ...
      if (!is.null(payload)) {
        ...
        if (!is.null(dict)) {
          blk <- encode_trained_dictionary_block(column$values$strings, dict)
