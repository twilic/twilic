...
final class TwilicCodec {
  ...
  private Message readMessage(Wire.Reader reader) {
    reader.enterDepth();
    try { return readMessageInner(reader); } finally { reader.leaveDepth(); }
  }

  private Message readMessageInner(Wire.Reader reader) {
    Message message = new Message();
    message.kind = parseMessageKind(Byte.toUnsignedInt(reader.readU8()));
    switch (message.kind) {
      case SCALAR -> message.scalar = readValue(reader);
      case ARRAY -> {
        long n = reader.readCount();
  ...
  private Value readValueWithField(Wire.Reader reader, String fieldIdentity) {
    reader.enterDepth();
    try { return readValueWithFieldInner(reader, fieldIdentity); } finally { reader.leaveDepth(); }
  }

  private Value readValueWithFieldInner(Wire.Reader reader, String fieldIdentity) {
  ...
  private Value readStringValue(Wire.Reader reader, String fieldIdentity) {
    ...
    return switch (mode) {
      ...
      case PREFIX_DELTA -> {
        ...
        long prefixLen = reader.readCount();
  ...
  private List<String> readStringVector(Wire.Reader reader, VectorCodec codec) {
    return switch (codec) {
      case DICTIONARY -> {
        long dictSize = reader.readCount();
  ...
  private ShapedObjectMessage readShapedObject(Wire.Reader reader) {
    ShapedObjectMessage shaped = new ShapedObjectMessage();
    shaped.shapeId = reader.readCount(65535);
  ...
  private RowBatchMessage readRowBatch(Wire.Reader reader) {
    ...
    long rowCount = reader.readCount();
    for (long i = 0; i < rowCount; i++) {
      long fieldCount = reader.readCount();
  ...
  private ColumnBatchMessage readColumnBatch(Wire.Reader reader) {
    ...
    batch.count = reader.readCount();
    long colCount = reader.readCount();
  ...
  private ControlMessage readControl(Wire.Reader reader) {
    ...
    switch (control.opcode) {
      ...
      case REGISTER_SHAPE -> {
        ...
        registerShape.shapeId = reader.readCount(65535);
        long n = reader.readCount();
  ...
  private TypedVector readTypedVector(
    ...
    ElementType elementType =
        forcedElement == null
            ...
            : forcedElement;
    long expectedLen = reader.readCount();
