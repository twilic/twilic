...
echo "[interop] Emitting Scala client frames..."
(cd "${ROOT_DIR}" && sbt -batch -error emitRustClientFixtures 2>/dev/null) | grep -E '^(codec|session)\|' > "${FIXTURES_FILE}"
...
echo "[interop] OK: Scala server -> Rust client smoke test passed"
