...
echo "[interop] Emitting Dart server frames..."
(cd "${ROOT_DIR}" && dart run bin/emit_rust_client_fixtures.dart) > "${FIXTURES_FILE}"
...
echo "[interop] OK: Dart server -> Rust client smoke test passed"
