...
internal object V2 {
    ...
    private fun decodeV2ArrayBody(reader: Wire.Reader, state: DecodeState, length: Int): Value {
        reader.claimOutput(length.toLong())
        reader.enterDepth()
        try { return decodeV2ArrayBodyInner(reader, state, length) } finally { reader.leaveDepth() }
    }

    private fun decodeV2ArrayBodyInner(reader: Wire.Reader, state: DecodeState, length: Int): Value {
        if (length == 0) {
            return Value.ofArray(emptyList())
        }
        val firstTag = reader.readU8().toInt() and 0xFF
        if (firstTag == SHAPE_DEF_TAG) {
            val shapeId = reader.readCount(65535)
            val keyCount = reader.readCount(256).toInt()
            val keys = mutableListOf<String>()
            repeat(keyCount) {
            ...
            repeat(length) {
                reader.claimOutput(keys.size.toLong())
    ...
    private fun decodeV2MapBody(reader: Wire.Reader, state: DecodeState, length: Int): Value {
        reader.claimOutput(length.toLong())
        reader.enterDepth()
        try { return decodeV2MapBodyInner(reader, state, length) } finally { reader.leaveDepth() }
    }

    private fun decodeV2MapBodyInner(reader: Wire.Reader, state: DecodeState, length: Int): Value {
