...
function u64_lo(int $v): int
{
    return $v & 0xFFFFFFFF;
}

function u64_hi(int $v): int
{
    return ($v >> 32) & 0xFFFFFFFF;
}

function append_u64_le_word(ByteBuffer $out, int $selector, int $payload): void
{
    $bytes = "\0\0\0\0\0\0\0\0";
    for ($i = 0; $i < 7; $i++) {
        $bytes[$i] = chr(($payload >> (8 * $i)) & 0xFF);
    }
    $bytes[7] = chr((($selector & 0x0F) << 4) | (($payload >> 56) & 0x0F));
    $out->appendBytes($bytes);
}

/** @return array{0: int, 1: int} */
function read_u64_le_word(Reader $reader): array
{
    $b = $reader->readExact(8);
    $selector = (ord($b[7]) >> 4) & 0x0F;
    $payload = 0;
    for ($i = 0; $i < 7; $i++) {
        $payload |= ord($b[$i]) << (8 * $i);
    }
    $payload |= (ord($b[7]) & 0x0F) << 56;
...
function encode_u64_simple8b_inner(array $values, ByteBuffer $out): void
{
    ...
    while ($idx < $n) {
        ...
        if ($zeroRun >= 120) {
            ...
            append_u64_le_word($out, $take === 240 ? 0 : 1, 0);
            $idx += $take;
            continue;
        }
        $packed = false;
        foreach (SIMPLE8B_SLOTS as $selectorIdx => $slot) {
            ...
            append_u64_le_word($out, $selector, $payload);
            $idx += $count;
            $packed = true;
            break;
        }
        if (!$packed) {
            append_u64_le_word($out, 15, $values[$idx] & ((1 << 60) - 1));
            $idx++;
        }
    }
}
...
function decode_u64_simple8b_inner(Reader $reader): array
{
    ...
    while (count($out) < $length) {
        [$selector, $payload] = read_u64_le_word($reader);
...
{
    $bytes = pack('V', $u & 0xFFFFFFFF) . pack('V', ($u >> 32) & 0xFFFFFFFF);
...
{
    $lo = u64_lo($a) + u64_lo($b);
    $carry = $lo > 0xFFFFFFFF ? 1 : 0;
    $lo &= 0xFFFFFFFF;
    $hi = u64_hi($a) + u64_hi($b) + $carry;
    if ($hi > 0xFFFFFFFF) {
        return [0, false];
    }
