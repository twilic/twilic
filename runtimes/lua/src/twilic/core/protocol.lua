...
function TwilicCodec.write_column(self, column, out)
  ...
  local trained_block
  if column.dictionary_id and column.values.kind == model.ELEMENT_TYPE_STRING then
    if column.codec == model.VECTOR_CODEC_DICTIONARY or column.codec == model.VECTOR_CODEC_STRING_REF then
      local payload = self.state.dictionaries[column.dictionary_id]
      if payload then
        local ok, dict = pcall(dictionary.decode_trained_dictionary_payload, payload)
        if ok then
          local block, block_ok = dictionary.encode_trained_dictionary_block(column.values.strings, dict)
          if block_ok then
            trained_block = block
          end
        end
...
function TwilicCodec.write_control(self, control, out)
  ...
  elseif control.opcode == model.CONTROL_OPCODE_REGISTER_SHAPE then
    if not control.register_shape then
      errors.raise(errors.invalid_data("register shape payload missing"))
    end
    wire.encode_varuint(control.register_shape.shape_id, out)
    wire.encode_varuint(#control.register_shape.keys, out)
    local key_names = {}
    for i = 1, #control.register_shape.keys do
      local key_ref = control.register_shape.keys[i]
      TwilicCodec.write_key_ref(self, key_ref, out)
      key_names[i] = ph.key_ref_string(key_ref, self.state)
    end
    session.shape_register_with_id(self.state.shape_table, control.register_shape.shape_id, key_names)
  elseif control.opcode == model.CONTROL_OPCODE_REGISTER_STRINGS then
    wire.encode_varuint(#control.register_strings, out)
    for i = 1, #control.register_strings do
      wire.encode_string(control.register_strings[i], out)
      session.intern_register(self.state.string_table, control.register_strings[i])
    end
  elseif control.opcode == model.CONTROL_OPCODE_PROMOTE_STRING_FIELD_TO_ENUM then
    local promote = control.promote_string_field_to_enum
    if not promote then
      errors.raise(errors.invalid_data("promote enum payload missing"))
    end
    wire.encode_string(promote.field_identity, out)
    wire.encode_varuint(#promote.values, out)
    for i = 1, #promote.values do
      wire.encode_string(promote.values[i], out)
    end
    self.state.field_enums[promote.field_identity] = promote.values
  ...
  elseif control.opcode == model.CONTROL_OPCODE_RESET_STATE then
    ...
    errors.raise(errors.invalid_data("control opcode"))
  end
end
...
function TwilicCodec.read_control(self, reader)
  ...
  elseif opcode == model.CONTROL_OPCODE_REGISTER_SHAPE then
    local shape_id = reader:read_count(65535)
    local n = reader:read_count()
...
function TwilicCodec.read_message(self, reader)
  return reader:with_depth(function() return TwilicCodec.read_message_inner(self, reader) end)
end

function TwilicCodec.read_message_inner(self, reader)
  ...
  elseif kind == model.MESSAGE_KIND_ROW_BATCH then
    local row_count = reader:read_count()
    local rows = {}
    for _ = 1, row_count do
      local field_count = reader:read_count()
      local row = {}
  ...
  elseif kind == model.MESSAGE_KIND_COLUMN_BATCH then
    local count = reader:read_count()
    local col_count = reader:read_count()
    local cols = {}
...
function TwilicCodec.read_value_with_field(self, reader, field_identity)
  return reader:with_depth(function() return TwilicCodec.read_value_with_field_inner(self, reader, field_identity) end)
end

function TwilicCodec.read_value_with_field_inner(self, reader, field_identity)
  ...
  if tag == M.TAG_STRING then
    ...
    if mode == model.STRING_MODE_PREFIX_DELTA then
      local base_id = reader:read_varuint()
      local prefix_len = reader:read_count()
...
function TwilicCodec.read_typed_vector(self, reader, forced_element, expected_codec)
  ...
  local expected_len = reader:read_count()
  local codec_byte = reader:read_u8()
...
function TwilicCodec.read_string_vector(self, reader, codec_id)
  if codec_id == model.VECTOR_CODEC_DICTIONARY then
    local dict_n = reader:read_count()
    local dct = {}
...
function SessionEncoder.encode(self, value)
  local msg = TwilicCodec.message_for_value(self.codec, value)
  if self.codec.state.options.enable_state_patch and self.codec.state.previous_message
      and ph.supports_state_patch(self.codec.state.previous_message, msg) then
    local ops = ph.diff_message(self.codec.state.previous_message, msg)
    local patch_msg = model.message({
      kind = model.MESSAGE_KIND_STATE_PATCH,
      state_patch = { base_ref = model.base_ref_previous(), operations = ops, literals = {} },
    })
    if ph.encoded_size(patch_msg) < ph.encoded_size(msg) then
      local ok, result = pcall(TwilicCodec.encode_message, self.codec, patch_msg)
      if ok then return result end
    end
  end
  return TwilicCodec.encode_message(self.codec, msg)
end

function SessionEncoder.encode_with_schema(self, schema, value)
  ...
  for i = 1, #schema.fields do
    ...
    if f.enum_values then self.codec.state.field_enums[f.name] = f.enum_values end
  end
  if value.kind ~= model.VALUE_MAP then
    errors.raise(errors.invalid_data('encode_with_schema expects map value'))
  end
  local presence, fields, has_presence = {}, {}, false
  for i = 1, #schema.fields do
    local f = schema.fields[i]
    local v = ph.lookup_map_field(value, f.name)
    if v then
      presence[i] = true
      fields[i] = model.clone_value(v)
    else
      presence[i] = false
      has_presence = true
    end
  ...
  local msg = model.message({
    ...
    schema_object = { schema_id = schema.schema_id, presence = presence, has_presence = has_presence, fields = fields },
  })
  return TwilicCodec.encode_message(self.codec, msg)
end

function SessionEncoder.encode_batch(self, values)
  local msg
  if #values == 0 then
    msg = model.message({ kind = model.MESSAGE_KIND_ROW_BATCH, row_batch = { rows = {} } })
  elseif #values >= 16 then
    local cols = ph.columns_from_map_values(values)
    if not cols then cols = ph.rows_to_columns(ph.rows_from_values(values)) end
    if self.codec.state.options.enable_trained_dictionary then
      dictionary.apply_dictionary_references(self.codec.state, cols)
    end
    msg = model.message({ kind = model.MESSAGE_KIND_COLUMN_BATCH, column_batch = { count = #values, columns = cols } })
  else
    msg = model.message({ kind = model.MESSAGE_KIND_ROW_BATCH, row_batch = { rows = ph.rows_from_values(values) } })
  end
  local data = TwilicCodec.encode_message(self.codec, msg)
  self.codec.state.previous_message = model.clone_message(msg)
  self.codec.state.previous_message_size = #data
  SessionEncoder.record_full_message_as_base(self)
  return data
end

function SessionEncoder.encode_patch(self, value)
  local msg = TwilicCodec.message_for_value(self.codec, value)
  if not self.codec.state.previous_message or not ph.supports_state_patch(self.codec.state.previous_message, msg) then
    return TwilicCodec.encode_message(self.codec, msg)
  end
  local ops = ph.diff_message(self.codec.state.previous_message, msg)
  local patch_msg = model.message({
  ...
  return TwilicCodec.encode_message(self.codec, patch_msg)
end

function SessionEncoder.encode_micro_batch(self, values)
  if #values == 0 then return SessionEncoder.encode_batch(self, values) end
  if not self.codec.state.options.enable_template_batch or not ph.has_uniform_micro_batch_shape(values) then
    return SessionEncoder.encode_batch(self, values)
  end
  local columns = ph.columns_from_map_values(values)
  if not columns then columns = ph.rows_to_columns(ph.rows_from_values(values)) end
  if self.codec.state.options.enable_trained_dictionary then
    dictionary.apply_dictionary_references(self.codec.state, columns)
  end
  local template_id, ok = ph.find_template_id(self.codec.state.templates, columns)
  if not ok then
    template_id = session.allocate_template_id(self.codec.state)
    self.codec.state.templates[template_id] = ph.template_descriptor_from_columns(template_id, columns)
    self.codec.state.template_columns[template_id] = columns
    local mask = {}
    for i = 1, #columns do mask[i] = true end
    local msg = model.message({
      kind = model.MESSAGE_KIND_TEMPLATE_BATCH,
      template_batch = { template_id = template_id, count = #values, changed_column_mask = mask, columns = columns },
    })
    return TwilicCodec.encode_message(self.codec, msg)
  end
  local mask, changed_cols = ph.diff_template_columns(self.codec.state.template_columns[template_id], columns)
  self.codec.state.template_columns[template_id] = columns
  local msg = model.message({
    kind = model.MESSAGE_KIND_TEMPLATE_BATCH,
    template_batch = { template_id = template_id, count = #values, changed_column_mask = mask, columns = changed_cols },
  })
  return TwilicCodec.encode_message(self.codec, msg)
end

function SessionEncoder.reset(self)
  session.reset_state(self.codec.state)
end

function SessionEncoder.decode_message(self, data)
  return TwilicCodec.decode_message(self.codec, data)
end

function SessionEncoder.record_full_message_as_base(self)
  if self.codec.state.options.max_base_snapshots == 0 then return end
  if not self.codec.state.previous_message then return end
  local base_id = session.allocate_base_id(self.codec.state)
  session.register_base_snapshot(self.codec.state, base_id, self.codec.state.previous_message)
end
