...
function M.checked_add_u64(a, b)
  ...
  b = b & model.MAX_U64
  local sum = a + b
  local masked = sum & model.MAX_U64
  if b ~= 0 and masked < a then
    return masked, false
  end
  return masked, true
end
...
function M.decode_u64_plain(reader)
  local length = reader:read_count()
  local out = {}
...
function M.decode_u64_rle(reader)
  local runs_len = reader:read_count()
  local out = {}
  for _ = 1, runs_len do
...
function M.f64_to_u64(value)
  return select(1, string.unpack("<I8", string.pack("<d", value)))
end

function M.u64_to_f64(bits)
  return select(1, string.unpack("<d", string.pack("<I8", bits)))
end
...
function M.decode_i64_patched_for(reader)
  ...
  local patch_count = reader:read_count()
